...
public class Client extends SuperActor implements Steppable {
    ...
    protected void handleCashIn(PaySim paysim, int step, double amount) {
        ...
        Transaction t = new Transaction(step, CASH_IN, amount, nameOrig, oldBalanceOrig,
                ...
                randomIp(rand), randomDevice(rand), randomBrowser(rand), randomCountry(rand), randomGeo(rand),
                randomSessionId(rand), randomLoginTime(rand), randomAccountType(rand), randomKycLevel(rand),
                this.bank.getName(), randomFraudScenario(rand), randomAiAlert(rand), randomExternalBlacklist(rand));
